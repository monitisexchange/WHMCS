...
class monitisSnapshotsClass {
	...
	/////////////////////////////////////////////////
	public function extSnapshotsIds() {
		$whmcs_ext = $this->whmcs_ext;
		$this->whmcs_ext_ids = $this->_idsList( $whmcs_ext, 'monitor_id' );
	}	
	public function allSnapshots( $client_id ) {
		$oWHMCS = new WHMCS_class( $client_id );
		
		$this->whmcs_ext = $oWHMCS->ext_monitors();
		
		if( $this->whmcs_ext ) {
			$this->extSnapshotsIds();
		}
		
		$this->whmcs_int = $oWHMCS->int_monitors();
...
<hr/>
<?
echo '<section class="section_block">';
if( $ext_ids )
